    ...
    if dataset == 'COMPAS':
	    if model == 'logistic_regression':
    		...
    		if postprocess == 'Equalize AP by deferring on one' :
				...
				command = 'Rscript'
    			path_to_rscript = '../compas_minPDF.R'
            elif postprocess == 'Equalize PPV and NPV using 2 thresholds':
               	command = 'Rscript'
    			path_to_rscript = '../compas_two_thresholds.R'
        ...
        if model == 'logistic_regression':
            ...
            if postprocess == 'Equalize AP by deferring on one' :
				...
				command = 'Rscript'
    			path_to_rscript = '../loan_minPDF.R'
            elif postprocess == 'Equalize PPV and NPV using 2 thresholds':
                command = 'Rscript'
    			path_to_rscript = '../loan_two_threshold.R'
            cmd = [command, path_to_rscript]
            subprocess.checkout(cmd)
    	...
    	if model == 'logistic_regression':
            ...
            if postprocess == 'Equalize AP by deferring on one' :
				...
				command = 'Rscript'
    			path_to_rscript = '../student_minPDF.R'
           	elif postprocess == 'Equalize PPV and NPV using 2 thresholds':
            	command = 'Rscript'
